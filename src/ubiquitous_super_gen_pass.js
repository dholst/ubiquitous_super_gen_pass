(function() {
  ...
  function hash(masterPassword, domain, length) {
		var passwordCandidate = masterPassword + ':' + domain;
		
		for(var i = 0; i < 10; ++i) {
		  passwordCandidate = b64_md5(passwordCandidate);
		}
		
		while( passwordCandidate.substr(0,length).search(/[a-z]/) != 0 || passwordCandidate.substr(0,length).search(/[A-Z]/) == -1 || passwordCandidate.substr(0,length).search(/[0-9]/) == -1) {
		  passwordCandidate = b64_md5(passwordCandidate);
		}
	  
		return passwordCandidate.substr(0,length);
	}

  //
  // end of core hash functions
  //


  function lengthFrom(length) {
  ...
  function getDomainFrom(url) {
    ...
    var hostName = url.match(domainIsolator);
  ...
  function currentLocation() {
    return CmdUtils.getDocumentInsecure().location.href
  }
  
  function currentDomain() {
    return getDomainFrom(CmdUtils.getDocumentInsecure().location.href) || "unknown domain";
  }
  ...
  function getPasswordFieldsToChange() {
    var passwordFields = jQuery('input[type="password"]', CmdUtils.getDocumentInsecure());
    var passwordFieldsToChange = [];
    ...
    return passwordFieldsToChange
  }

  function getMasterPasswordsFromPasswordFields() {
    var passwordFields = getPasswordFieldsToChange();
    var passwords = [];

    for(var i = 0, j = passwordFields.length; i < j; ++i) {
      passwords.push(passwordFields[i].value);
    }

    return unique(passwords);
  }

  function setPasswordsOnPasswordFields(newPassword) {
    var passwordFields = getPasswordFieldsToChange();

    for(var i = 0, j = passwordFields.length; i < j; ++i) {
      passwordFields[i].value = newPassword;
      jQuery(passwordFields[i]).css('background-color', '#ada');
    }
  }

  var PREVIEW_TEMPLATE = 
    '<span style="color: #ff5; font-family: Helvetica;">' + 
      'Replace <strong>${number}</strong> password${plural} ' +
      'with a <strong>${length}</strong> digit SuperGenPass(word) ' + 
      'for <strong>${domain}</strong>.' + 
    '</span>';

  var NO_PASSWORD_MESSAGE = 
    '<span style="color: #f55; font-family: Helvetica;">' +
      'A password field was not found on this page or the master password was not entered in it.' + 
    '</span>';

  var MULTI_PASSWORD_MESSAGE = 
    '<span style="color: #f55; font-family: Helvetica;">' +
      'Multiple password fields found, but they contain different values. Make sure all passwords to be replaced are the same.' + 
    '</span>';

  CmdUtils.CreateCommand({
    name: 'super-gen-pass',
    takes: {domain: noun_arb_text},
    modifiers: {length: noun_arb_text},
    description: "Replaces password fields with the SuperGenPass'd password.",

    preview: function(pblock, domain, mods) {
      var masterPasswords = getMasterPasswordsFromPasswordFields();

      if(masterPasswords.length == 1) {
        var numberToChange = getPasswordFieldsToChange().length;

        pblock.innerHTML = CmdUtils.renderTemplate(PREVIEW_TEMPLATE, {
          number: numberToChange,
          plural: (numberToChange > 1 ? 's' : ''),
          length: lengthFrom(mods.length.text),
          domain: getDomainFrom(domain.text || currentLocation())
        });
      }
      else if(masterPasswords.length > 1) {
        pblock.innerHTML = MULTI_PASSWORD_MESSAGE;
      }
      else {
        pblock.innerHTML = NO_PASSWORD_MESSAGE;
      }
    },
      
    execute: function(domain, mods) {
      var masterPasswords = getMasterPasswordsFromPasswordFields();

      if(masterPasswords.length == 1) {
        var newPassword = hash(masterPasswords[0], getDomainFrom(domain.text || currentLocation()), lengthFrom(mods.length.text));
        setPasswordsOnPasswordFields(newPassword);
      }
    }
  });
})();
